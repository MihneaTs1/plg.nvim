...
-- internal: topo-sort plugins + deps
...
function M.install()
  ...
  -- 2) Clone missing (batch unpinned, individual pinned)
  local jobs, batch = {}, {}
  ...
  if #batch > 0 then
    local lines = {}
    for _, e in ipairs(batch) do lines[#lines+1] = e.url.." "..e.tgt end
    local cmdline = "printf '" .. table.concat(lines, "\n") ..
      "' | xargs -P4 -n2 sh -c 'git clone --depth=1 \"$0\" \"$1\"'"
    table.insert(jobs, fn.jobstart({ "sh","-c", cmdline }))
  end

  if #jobs > 0 then fn.jobwait(jobs, -1) end

  -- 3) packadd+config (deferred for start, lazy hooks for opt)
  for _, item in ipairs(ordered) do
...
-- internal async finder for outdated plugins
local function async_find_outdated(ordered, cb)
  ...
  for _, item in ipairs(ordered) do
    ...
    if fn.isdirectory(tgt)==1 then
      local cmdstr = "git -C "..tgt.." fetch --quiet && git -C "..tgt..
                     " rev-list --count HEAD..@{u}"
      fn.jobstart(cmdstr, {
        stdout_buffered = true,
        on_stdout = function(_, d)
          if tonumber(d[1]) and tonumber(d[1])>0 then out[#out+1]=item end
        end,
        on_exit = function() pending = pending-1; if pending==0 then cb(out) end end,
      })
    else
      pending = pending-1
